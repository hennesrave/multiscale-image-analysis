...
void HistogramViewer::paintEvent( QPaintEvent* event )
{
	...
	if( edges && counts )
	{
		...
		{
			const auto percentage = counts->value( hovered_object_global.bin ) / static_cast<double>( _database.dataset()->element_count() );
			labels_string += "Bin:";
			values_string += QString::number( 100.0 * percentage, 'f', 1 ) + " % of dataset";
		}

		if( hovered_object_segmentation.rectangle.width() && segmentation_counts )
		{
			const auto segment = _database.segmentation()->segment( hovered_object_segmentation.segment_number );
			const auto count = segmentation_counts->value( hovered_object_segmentation.segment_number )[hovered_object_segmentation.bin];
			const auto percentage_segment = count / static_cast<double>( segment->element_count() );
			const auto percentage_bin = count / static_cast<double>( counts->value( hovered_object_segmentation.bin ) );
	...
	if( const auto element_index = _database.highlighted_element_index(); element_index.has_value() )
	{
		if( const auto feature = _histogram.feature() )
		{
			...
			{
				const auto xscreen = this->world_to_screen_x( feature_values->value( *element_index ) );
